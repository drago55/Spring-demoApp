...
public class StorageServiceImpl implements StorageService {
    ...
    public String getFileName(MultipartFile file) {
        return StringUtils.cleanPath(UUID.randomUUID().toString() + "." + StringUtils.getFilenameExtension(file.getOriginalFilename()));
    }
    ...
    public Stream<Path> loadAll() {
        try {
            return Files.walk(this.storedLocation, 1)
                    .filter(path -> !path.equals(this.storedLocation))
